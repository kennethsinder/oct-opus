...
def train_epoch(tbd_writer, train_dataset, model_state, epoch_num):
    gen_total_loss_sum, gen_gan_loss_sum, gen_l1_loss_sum, disc_loss_sum = 0, 0, 0, 0
    index = 0
    for input_image, target in train_dataset:
        ...
        gen_l1_loss_sum += gen_l1_loss
        disc_loss_sum += disc_loss
        index += 1

        # log info to tensorboard
        with tbd_writer.as_default():
            tf.summary.scalar('avg_gen_total_loss', gen_total_loss_sum / index, step=epoch_num)
            tf.summary.scalar('avg_gen_gan_loss', gen_gan_loss_sum / index, step=epoch_num)
            tf.summary.scalar('avg_l1_loss', gen_l1_loss_sum / index, step=epoch_num)
