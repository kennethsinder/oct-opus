...
```
python cgan.py train -e {number_of_epochs} -d {top_level_data_directory}
```
...
If you are using our Sharcnet `sbatch` job files as a starting point, please remember to change all usernames and account names to the correct value (e.g. `def-<account name>`).
